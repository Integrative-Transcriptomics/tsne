...
d <- read.csv('/local_scratch.old/zabel/tsne/diss/datasets/Scoelicolor/wt_featurecounts_log_qnorm.txt', sep = '\t', header = 1)
selected_genes <- d[, order(apply(d[-1, ], 2, var), decreasing=TRUE)][, 1:50]
selected_genes_m <- as.matrix(selected_genes)

repli=rep(3,8)
results <- knorm(selected_genes_m, 25, 0.01, 1e-10, 1e-10, 2, length(repli),ncol(selected_genes_m),repli)

a_cor_est <- results$a_cor_est
g_cor_est <- results$g_cor_est
mean <- results$m_est

write.csv(selected_genes_m, '/local_scratch.old/zabel/tsne/diss/datasets/Scoelicolor/selected_data.txt', row.names = FALSE)
write.csv(mean, '/local_scratch.old/zabel/tsne/diss/datasets/Scoelicolor/mean.txt', row.names = FALSE)
write.csv(a_cor_est, '/local_scratch.old/zabel/tsne/diss/datasets/Scoelicolor/cov1.txt', row.names = FALSE)
write.csv(g_cor_est, '/local_scratch.old/zabel/tsne/diss/datasets/Scoelicolor/cov2.txt', row.names = FALSE)
